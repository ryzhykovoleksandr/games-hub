<script lang="ts">
  import { onlineCount, theme } from '../stores';

  const countDisplay = $derived($onlineCount.toLocaleString() + ' online');
</script>
...
<nav class="topbar">
  ...
  <ul class="nav-links">
    <li><a href="#hub" class="active">Hub</a></li>
...
<style>
  ...
  .logo span { color: var(--accent-prime); }
  .nav-links {
    display: flex;
    gap: 28px;
    list-style: none;
  ...
  .nav-links a.active { color: var(--accent-prime); }
  .nav-right {
    display: flex;
    align-items: center;
    gap: 16px;
